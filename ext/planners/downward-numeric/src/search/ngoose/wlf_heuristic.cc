...
WlfHeuristic::WlfHeuristic(const options::Options &opts)
  ...
  min_pairs_list_ = model.attr("get_min_pairs_list")()
                        .cast<std::vector<std::vector<std::pair<int, int>>>>();
  ...
  std::cout << "min_pairs_list.size=" << min_pairs_list_.size() << std::endl;
  for (int i = 0; i < n_models_; i++) {
    std::cout << "min_pairs_" << i << ".size=" << min_pairs_list_[i].size()
              << std::endl;
  }
...
WlfHeuristic::compute_features(const ngoose_wlf_graph::WlfGraph &state_graph) {
  ...
  start_time = get_time();
  CatFeature cat_0(n_nodes, 0);
  CatFeature cat_1(n_nodes, 0);
  ...
  for (int itr = 0; itr < cat_iterations_; itr++) {
    if (itr % 2 == 0) {
      for (size_t u = 0; u < n_nodes; u++) {
        ...
        new_colour = std::to_string(cat_0[u]);
      ...
      for (size_t u = 0; u < n_nodes; u++) {
        ...
        // add current colour and sorted neighbours into sorted colour key
        new_colour = std::to_string(cat_1[u]);
        for (const auto &ne_pair : neighbour_cats) {
          new_colour += "," + std::to_string(ne_pair.first) + "," +
                        std::to_string(ne_pair.second);
        }
...
ap_float WlfHeuristic::estimate(const std::vector<ap_float> x, int model_index) {
  ...
  for (size_t j = 0; j < min_pairs_list_[model_index].size(); j++) {
    const int a = min_pairs_list_[model_index][j].first;
    const int b = min_pairs_list_[model_index][j].second;
    h += std::min(x[a], x[b]) * weights_list_[model_index][n_features_ + j];
  }
  end_time = get_time();
