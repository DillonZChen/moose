...
def _ground_term(term: Term, mapping: dict[Variable, Constant]) -> Constant:
    # NOTE the pddl package does not seem to handle constant objects in numeric expressions correctly.
    # https://github.com/AI-Planning/pddl/issues/134
    # Thus, we assume anything that is not in mapping is a constant.

    key = str(term)
    if isinstance(term, Constant) or key not in mapping:
        return term
    elif isinstance(term, Variable):
        return mapping[key]
        # try:
        #     return mapping[str(term)]
