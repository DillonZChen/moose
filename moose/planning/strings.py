...
from pddl.logic.functions import EqualTo, GreaterEqualThan, GreaterThan, NumericFunction, NumericValue

from moose.planning import State, get_domain, get_problem
...
Condition = ClassicCondition | NumericCondition
...
def condition_to_string(p: str | ClassicCondition | NumericCondition) -> str:
    if isinstance(p, Predicate | NumericFunction):
        ...
        return token_to_numeric_value(p)
...
def state_to_string(state: State | list[str], delimiter: str = ",", sort: bool = True) -> str:
    state_str = []
    for p in state:
...
def aopt_to_string(schema_opt: tuple[str, list[Any]]) -> str:
    return f"{schema_opt[0]}({','.join(map(str, schema_opt[1]))})"


def aopt_to_stringB(schema_opt: tuple[str, list[Any]]) -> str:
    return f"({' '.join([schema_opt[0]] + list(map(str, schema_opt[1])))})"
...
def token_to_numeric_value(token: lark.lexer.Token | float | int) -> float | int:
